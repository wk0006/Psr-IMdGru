...
def mdlstm_matrix(X_train,X_test,y_train,y_test):
    ...
    hidden = layers.GRU(128,activation ='relu')(Input)
...
#     os = layers.GRU(NHIDDEN3)(os)
    ...
    history=model.fit(X_train1,y_train1,epochs=100,batch_size=32,verbose=2)
...
def mdlstm_values(X_train,X_test,y_train,y_test):
    from sklearn.preprocessing import StandardScaler
    x_scaler = StandardScaler()
    y_scaler = StandardScaler()
...
#     os = layers.GRU(NHIDDEN3)(os)
    ...
    model = keras.Model(Input,Output)
    model.compile(optimizer='adam',loss=loss_func,metrics=[loss_func])
    history=model.fit(X_train1,y_train1,epochs=200,batch_size=32,verbose=2)
    out_pct_test, out_mu_test, out_std_test = get_mixture_coef(model.predict(X_test1), Training=False)
    out_new = model.predict(np.array(X_test1))
    pct_new, mu_new, std_new = get_mixture_coef(out_new, Training=False)
    r_mu =  pct_new*mu_new
